...
\newcommand{\fundingParameters}{%
  The following parameters can be used:

\begin{tabularx}{\textwidth}{*{3}{!{\VRule[-1pt]}l}!{\VRule[-1pt]}X}
  \headrow
  \textbf{Parameter} & \textbf{Data type} & \textbf{Required} & \textbf{Description}\\
  \t{uuid}           & \t{string}         & No                & The uuid of an existing funding
                                                                record.  When this parameter is
                                                                set, other parameters will be
                                                                ignored.\\
  \t{title}          & \t{string}         & Yes               & The title of the funding project.\\
  \t{grant\_code}    & \t{string}         & No                & An optional grant code of the funding.\\
  \t{funder\_name}   & \t{string}         & No                & The name of the funder.\\
  \t{url}            & \t{string}         & No                & A URL to the funding project or
                                                                funding organization.
\end{tabularx}}

\newcommand{\authorParameters}{%
  The following parameters can be used:

\begin{tabularx}{\textwidth}{*{3}{!{\VRule[-1pt]}l}!{\VRule[-1pt]}X}
  \headrow
  \textbf{Parameter} & \textbf{Data type} & \textbf{Required} & \textbf{Description}\\
  \t{full\_name}     & \t{string}         & No                & The full name of the author.\\
  \t{first\_name}    & \t{string}         & Yes               & The first name of the author.\\
  \t{last\_name}     & \t{string}         & Yes               & The last name of the author.\\
  \t{email}          & \t{string}         & No                & The e-mail address of the author.\\
  \t{orcid\_id}      & \t{string}         & No                & The ORCID identifier for the author.\\
  \t{job\_title}     & \t{string}         & No                & The job title of the author.
\end{tabularx}}
...
\subsection{\t{/v3/datasets/<git-uuid>.git} (\t{GET})}
  ...
  This endpoint can be used to pass as a URL to \code{git clone}.
...
\label{sec:api-v2-articles-categories-post}
  ...
  by \code{dataset-id}.  \categoriesParameters
...
\label{sec:v2-account-collection-articles}
  ...
  \refer{sec:api-v2-articles-search-post} shows how to obtain the dataset
  identifiers.
