...
\begin{tabularx}{\textwidth}{*{1}{!{\VRule[-1pt]}l}!{\VRule[-1pt]}X}
  ...
  \t{sparql-update-uri}      & The URI at which the SPARQL 1.1 Update endpoint
                               ...
                               the \t{sparql-uri}.\\
  \t{seconds-to-wait-for-online} & Number of seconds to retry in case the SPARQL
                               endpoint is down when initializing. A retry is
                               done every second, and after the configured
                                   amount, \t{djehuty} will give up.\\
  \t{read-only-mode}         & When set to \code{1}, no queries that change
                               the state of the database shall be made.  This
                               is useful for running secondary instances to
                               balance the load of specific endpoints. Defaults
                               to \code{0}.
\end{tabularx}
...
\begin{tabularx}{\textwidth}{*{1}{!{\VRule[-1pt]}l}!{\VRule[-1pt]}X}
  \headrow
  \textbf{Option}            & \textbf{Description}\\
  \t{delay-inserting-log-entries} & When set to 1, it writes the \code{INSERT}
                               queries for log entries only to the audit log,
                               rather than executing them on the SPARQL
                               endpoint.\\
...
\section{Running \t{djehuty} with feature isolation}
...
\subsection{Isolating Git operations}
\label{sec:isolating-git-ops}
...
by defining multiple \code{location} blocks in the \t{nginx} configuration.

So we create two configuration files, differing only in the \code{port},
\code{log-file} and \code{rdf-store/read-only-mode} options.
\begin{itemize}
\item{djehuty-primary.xml}: Set the \code{rdf-store/read-only-mode} to
  \code{0} and \code{port} to \code{8080}.
\item{djehuty-git.xml}: Set the \code{rdf-store/read-only-mode} to
  \code{1} and \code{port} to \code{8081}.
\end{itemize}
...
The accompanying \t{nginx} \t{server}-block configuration looks as following:
\begin{lstlisting}
server {
    listen              443 ssl;
    listen              [::]:443 ssl;
...
\end{lstlisting}
  ...
  section \refer{sec:isolating-git-ops}).
