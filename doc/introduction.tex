...
\code{djehuty} is a data and software repository system.  The name finds its
inspiration in \href{https://en.wikipedia.org/wiki/Thoth}{Thoth}, the Egyptian
...
\label{sec:obtaining-tarball}
  ...
  \href{\djehutygiturl/releases}%
  {Releases}%
  \footnote{\url{\djehutygiturl/releases}}
...
\begin{tabularx}{\textwidth}{*{1}{!{\VRule[-1pt]}l}!{\VRule[-1pt]}X}
  ...
  \t{latest}          & This image points to the latest \t{djehuty} release.
                        ...
                        codebase.\\
  \t{XX.X}            & Release number where the number before the dot refers
                        to the year and the number after the dot refers to
                        the month.  Use a specific version image when you
                        want to upgrade at your own pace.
\end{tabularx}
...
\begin{tabularx}{\textwidth}{*{1}{!{\VRule[-1pt]}l}!{\VRule[-1pt]}X}
  ...
  \href{\djehutygiturl/releases/download/v\djehutyversion/djehuty-\djehutyversion-1.el10.noarch.rpm}%
  {djehuty-\djehutyversion-1.el10.noarch.rpm} & Binary RPM, to install and run \t{djehuty}.\\
  \href{\djehutygiturl/releases/download/v\djehutyversion/djehuty-\djehutyversion-1.el10.src.rpm}%
  {djehuty-\djehutyversion-1.el10.src.rpm} & Source RPM, to (re)build from source code.
\end{tabularx}
